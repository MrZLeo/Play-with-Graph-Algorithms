#![allow(dead_code)]
use std::{collections::HashMap, fmt::Display, fs};

/// Weighted Graph without consideration of direction
#[derive(Debug, Clone)]
pub struct WeightedGraph {
    e: usize,
    v: usize,
    adj_list: Vec<HashMap<usize, i32>>,
    directed: bool,
}

impl WeightedGraph {
    pub fn from_file(path: &str, directed: bool) -> Self {
        let file = match fs::read_to_string(path) {
            Err(_) => panic!("Cannot open file {}", path),
            Ok(str) => str,
        };

        let mut lines = file.lines();
        let mut first_line = lines.next().unwrap().split_whitespace();
        let v = first_line.next().unwrap().parse::<usize>().unwrap();
        let e = first_line.next().unwrap().parse::<usize>().unwrap();

        let mut adj_list: Vec<HashMap<usize, i32>> = vec![HashMap::new(); v];

        // we can't use `self.validate_vertex` here, so we have to define
        // a closure to do that
        let validate_vertex = |x| {
            if x >= v {
                panic!("vertex {} is not valid", x);
            }
        };

        for line in lines {
            let mut line = line.split_whitespace();
            let x = line.next().unwrap().parse::<usize>().unwrap();
            let y = line.next().unwrap().parse::<usize>().unwrap();
            let weight = line.next().unwrap().parse::<i32>().unwrap();

            validate_vertex(x);
            validate_vertex(y);

            if x == y {
                panic!("Self Loop is Detected!");
            }

            if adj_list[x].contains_key(&y) {
                panic!("Parallel Edges are Detected!");
            }

            adj_list[x].insert(y, weight);
            if !directed {
                adj_list[y].insert(x, weight);
            }
        }

        WeightedGraph {
            e,
            v,
            adj_list,
            directed,
        }
    }

    pub fn from_file_undirected(path: &str) -> Self {
        Self::from_file(path, false)
    }

    pub fn new(v: usize, directed: bool) -> Self {
        Self {
            e: 0,
            v,
            adj_list: vec![HashMap::new(); v],
            directed,
        }
    }

    pub fn new_rg(&self) -> Self {
        let e = self.e * 2;
        let v = self.v;
        let mut adj_list = self.adj_list.clone();
        let directed = self.directed;

        for v in 0..v {
            for w in self.adj_edge(v) {
                adj_list[w].insert(v, 0);
            }
        }

        Self {
            e,
            v,
            adj_list,
            directed,
        }
    }

    #[inline]
    pub fn validate_vertex(&self, v: usize) {
        if v >= self.v {
            panic!("vertex {} is not valid", v);
        }
    }

    #[inline]
    pub fn e(&self) -> usize {
        self.e
    }

    #[inline]
    pub fn v(&self) -> usize {
        self.v
    }

    pub fn has_edge(&self, v1: usize, v2: usize) -> bool {
        self.validate_vertex(v1);
        self.validate_vertex(v2);
        self.adj_list[v1].contains_key(&v2)
    }

    pub fn add_edge(&mut self, v: usize, w: usize, weight: i32) {
        self.validate_vertex(v);
        self.validate_vertex(w);
        self.adj_list[v].insert(w, weight);
    }

    pub fn get_weight(&self, v: usize, w: usize) -> Option<i32> {
        if self.has_edge(v, w) {
            Some(*self.adj_list[v].get(&w).unwrap())
        } else {
            None
        }
    }

    pub fn set_weight(&mut self, v: usize, w: usize, weight: i32) {
        if self.has_edge(v, w) {
            self.adj_list[v].entry(w).and_modify(|x| *x = weight);
        }
    }

    // pub fn adj_edge(&self, v: usize) -> Box<dyn Iterator<Item = &usize> + '_> {
    pub fn adj_edge(&self, v: usize) -> Box<dyn Iterator<Item = usize>> {
        self.validate_vertex(v);
        Box::new(self.adj_list[v].clone().into_keys())
    }

    #[inline]
    pub fn directed(&self) -> bool {
        self.directed
    }
}

impl Display for WeightedGraph {
    fn fmt(&self, f: &mut std::fmt::Formatter<'_>) -> std::fmt::Result {
        let mut list = String::new();
        for v in 0..self.v {
            list.push_str(format!("{}: ", v).as_str());
            for (w, weight) in self.adj_list[v].iter() {
                list.push_str(format!("({w}: {weight}) ").as_str());
            }
            list.push('\n');
        }
        write!(
            f,
            "AdjList: \nV: {}, E: {}, Directed: {}\nList: \n{}",
            self.v, self.e, self.directed, list
        )
    }
}
